...
print("1hlixed's automatic BRSTM downloader v2")
import urllib.request
import re
import os
import shutil

docfilename = "TPP Music  - Tabellenblatt1.csv"
...
try:
	with open(docfilename) as f:
		...
		doclines= [line.split(",") for line in doclines]
except FileNotFoundError as e:
	print("===ERROR===\n" + docfilename + " not found. To use this program, Use File -> Download As -> CSV to download the music doc and save '"+docfilename + "' in the same folder as this program.")
	input("\n\nPress enter to exit.")
	exit()
...
try:
	...
	for i in dlrange:
		...
		if (statusArr[i-dlrange.start+1][0:3] == "[ ]"): #don't redownload if already downloaded
			...
			#only accept songs that have team decisions
			teamdecision = doclines[i][4].strip().lower()
			if not (('accept' in teamdecision) or ('change:' in teamdecision)):
				print("Line "+str(i)+" had a team decision of '"+teamdecision+"', skipping...")
			...
			try:
				...
				print("Error! Unable to scrape game name from smashcustommusic for line "+str(i))
				continue
